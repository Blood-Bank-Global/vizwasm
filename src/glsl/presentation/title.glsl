...
void pass0(out vec4 color) {
    ...
    vec2 uv = src_coord.xy/2 * iResolution.xy;

    if (font_willow(uv, TITLE_POS, title1, 0, title1_length)
        || font_willow(uv, TITLE_POS + vec2(0.0, float(font_willow_height)), title2, 0, title2_length)
        || font_willow(uv, SUBTITLE_POS, subtitle, 0, subtitle_length)) {
        color = vec4(1.0);
    }
}
