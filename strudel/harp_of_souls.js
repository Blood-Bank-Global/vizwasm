...
$: "c5".add.squeeze(rand.range(0, 4).floor().seg(8).degradeBy(.5)).sometimes(ply(2)).note().s("square")
    .att(.1).dec(.1).sus(.25).rel(.75).room(.4).roomsize(1).delay(0.05).delayfb(0.05).hpf(1000).lfo({ r: 8 }).lpf(1500)
    .rib("<0@4 0@4 0@4 8@4>", "<4@4 4@4 8@8>")
    ._pianoroll().gain(0).bmod({ bus: 1 });

$: "c4".add.squeeze(rand.range(0, 4).floor().seg(8).degradeBy(.5)).sometimes(ply(2)).note().s("square")
    .att(.05).dec(.1).sus(.25).rel(.75).room(.4).roomsize(2).delay(.1).delayfb(.2).hpf(500).lpf(1000).lfo({ r: 8 })
...
$: ccv(isaw.seg(32)).ccn(0).midichan(1).midi('IAC Driver Bus 1');
$: ccv("[[.5 .45]!4]").ccn(17).midichan(5).midi('IAC Driver Bus 1');
$: ccv("[6 4 2 0] - - -".div(127)).ccn(2).midichan(5).midi('IAC Driver Bus 1');
